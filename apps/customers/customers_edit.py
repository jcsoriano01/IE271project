                                ...
                                dbc.Form(
                                    dbc.Row(
                                        [
                                            ...
                                            ,dbc.Col(
                                                dbc.Input(type = "text", id = 'customers_search_last_name',  placeholder="Last Name")
                                                ,className="me-3"
                                            )
                                            ,dbc.Label("Middle Name",  width="auto")
                                            ,dbc.Col(   
                                                dbc.Input(type = "text", id = 'customers_search_middle_name', placeholder="Middle Name")
                                                ,className="me-3"
                                            )
                                            ,dbc.Label("First Name", width="auto")
                                            ,dbc.Col(
                                                dbc.Input(type = "text", id = 'customers_search_first_name', placeholder="First Name")
                                                ,className="me-3"
                                            )
...
@app.callback(
    [
        Output('user_record_search_result','children')
    ],
    [
        Input('url','pathname')
        ,Input('customers_search_user_id', 'value')
        ,Input('customers_search_last_name', 'value')
        ,Input('customers_search_middle_name', 'value')
        ,Input('customers_search_first_name', 'value')
        #,Input('customers_search_btn', 'n_clicks')
    ]
)
def customers_loadlist(pathname, user_id, last_name, middle_name, first_name):
    if pathname == '/customer_registration/edit':
        sql = """
            SELECT
                a.created_date
                ,a.user_id
            ...
            for user_id in df['user_id']:
                buttons += [
                    html.Div(
                        dbc.Button(
                            'Edit'
                            ,href=f'/customer_registration/new?mode=edit&id={user_id}'
                            ,size = 'sm'
                            ,color = 'info'
                            ,className="me-1"
