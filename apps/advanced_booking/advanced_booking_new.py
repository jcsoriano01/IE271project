                ...
                ,dbc.CardBody(
                    [
                                ...
                                dbc.Form(
                                    dbc.Row(
                                        [
                                            ...
                                            ,dbc.Col(
                                                dbc.Input(type = 'text', id = 'ab_search_user_id', placeholder="Enter User ID")
                                                ,className='me-3'
                                            )
                                            ,dbc.Label("Last Name", width="auto")
                                            ,dbc.Col(
                                                dbc.Input(type = "text", id = 'ab_search_last_name',  placeholder="Last Name")
                                                ,className="me-3"
                                            )
                                            ,dbc.Label("Middle Name",  width="auto")
                                            ,dbc.Col(   
                                                dbc.Input(type = "text", id = 'ab_search_middle_name', placeholder="Middle Name")
                                                ,className="me-3"
                                            )
                                            ,dbc.Label("First Name", width="auto")
                                            ,dbc.Col(
                                                dbc.Input(type = "text", id = 'ab_search_first_name', placeholder="First Name")
                                                ,className="me-3"
                                            )
                                            ,dbc.Col(dbc.Button("Search",color="primary", n_clicks=0, id = 'ab_search_btn'),width="auto")
                                        ]
                                        ,className= "g-2"
                                    )
                                )
                        ...
                        ,html.Div(
                            ...
                            ,id = 'ab_user_record_search_result'
                        )
                    ]
                )
            ]
...
@app.callback(
    [
        Output('ab_user_record_search_result','children')
    ],
    [
        Input('url','pathname')
        ,Input('ab_search_user_id', 'value')
        ,Input('ab_search_last_name', 'value')
        ,Input('ab_search_middle_name', 'value')
        ,Input('ab_search_first_name', 'value')
        #,Input('customers_search_btn', 'n_clicks')
    ]
)
def customers_loadlist(pathname, user_id, last_name, middle_name, first_name):
    if pathname == '/advanced_booking/new':
        sql = """
            SELECT
                a.created_date
                ,a.user_id
            ...
            for user_id in df['user_id']:
                buttons += [
                    html.Div(
                        dbc.Button(
                            'Book'
                            ,href=f'/advanced_booking/new/book?mode=edit&id={user_id}'
                            ,size = 'sm'
                            ,color = 'info'
                            ,className="me-1"
