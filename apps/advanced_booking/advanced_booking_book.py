    ...
    [
        ...
        dbc.Modal(
            [
                ...
                dbc.ModalBody("Your advance booking request has been successfully submitted. Please wait for the approval and confirmation of your booking."),
                dbc.ModalFooter(
                    dbc.Button("Close", id="close", className="ms-auto", n_clicks=0)
                ),
            ],
            id="advanced_booking_modal",
            is_open=False,
        ),
        dbc.Row(
            [
                dbc.Col(
                    html.Div(
                        [
                            html.Div(
                                html.Img(id='ab_customer_image_result',
                            ...
                            dbc.Col(
                                        [
                                            dbc.Card(
                                                [
                                                    dbc.CardHeader(
                                                        [html.H5("Remaining Booking Credit", style={'margin': 0})],
                                                        style={'display': 'flex', 'align-items': 'center', 'justify-content': 'center', 'padding': '10px','background-color':'primary','fontFamily': 'Georgia'}
                                                    ),
                                                    dbc.CardBody(
                                                        [
...
@app.callback(
    [
        Output('selected_parking_lot','data')
    ],
    [
        Input(f'parking_lot_{i * 3 + j + 1}', 'n_clicks') for i in range(5) for j in range(3)
    ],
    [
        Input('advanced_booking_submit','n_clicks')
    ]
)
def update_selected_parking_lot(*args):
    ...
    if "parking_lot_" in button_id:
        print(f'parking lot button_id: {button_id}')
        return [button_id]
    else:
...
lot_buttons = [f'parking_lot_{i * 3 + j + 1}' for i in range(5) for j in range(3)]
...
@app.callback(
    ...
    [
        ...
        Input('advanced_booking_floor','value'),
        Input('time_picker_start','value'),
        Input('time_picker_end','value'),
    ],
)
def update_parking_lot_availability(date, floor, park_time_start, park_time_end):
    ...
    for button_id in lot_buttons:
        lot_no = button_id.replace("parking_lot_","")
...
@app.callback(
    [
        Output('advanced_booking_modal','is_open'),
        #Output('approval-trigger','data'),
    ],
    [
        Input('advanced_booking_submit', 'n_clicks'),
        Input('url', 'pathname'),
        Input('selected_parking_lot', 'data'),
        Input("close","n_clicks"),
    ],
    [
        State('date_picker', 'date'),
        State('advanced_booking_floor', 'value'),
        State('time_picker_start','value'),
        State('time_picker_end','value'),
        State('url','search'),
        #State('approval-trigger','data')
    ]
    
)
#def handle_submit(submitbtn, pathname, date, floor, selected_lot, park_start, park_end, search):
def handle_submit(submitbtn, pathname, selected_lot,close_clicks,date, floor, park_start, park_end, search):
    ...
    if button_id == 'close':
            return [False]
    print(f'submit_button_id: {button_id}')

    eventid = ctx.triggered[0]['prop_id'].split('.')[0]
    if eventid != 'advanced_booking_submit' or not submitbtn:
    ...
    if selected_lot:
        selected_lot = selected_lot.replace("parking_lot_", "")

    print(f"user_id: {user_id}, selected_lot: {selected_lot}, date: {date}, floor: {floor}, park_start: {park_start}, park_end: {park_end}")
    #print(f"user_id: {user_id}, date: {date}, floor: {floor}, park_start: {park_start}, park_end: {park_end}")
    ...
    sql4 = """
        INSERT INTO booking(booking_id, registration_no, parking_id, booking_date, start_date, end_date,is_advanced_booking, is_adv_booking_apprv, adv_booking_applied_date)
        VALUES(%s, %s,%s,%s,%s,%s,%s,%s,%s)
    """
    values_4 = [str(max_booking_no),str(reg_no),str(parking_id),date,park_start, park_end,True,1,current_timestamp]
    #cols_4 = ['booking_id','registration_no','parking_id','booking_date','start_date','end_date','is_advanced_booking']
...
@app.callback(
    [
        Output('ab_customer_image_result', 'src'),
        Output('ab_customer_name', 'children'),
        Output('ab_customer_created_date', 'children'),
        Output('ab_customer_approval_date', 'children'),
        Output('ab_customer_car_brand', 'children'),
        Output('ab_customer_car_model', 'children'),
        Output('ab_customer_plate_number', 'children'),
        Output('ytd_advanced_booking', 'children'),
        Output('mtd_advanced_booking', 'children'),
        Output('remaining_booking_credit', 'children')
    ],
    [
        Input('url', 'pathname')
    ],
...
def advanced_book_customerprofile_load(pathname, search):
    if pathname == '/advanced_booking/new/book':
        parsed = urlparse(search)
        ...
        if to_load == 1:
            ...
            remaining_credit = 5 - mtd_count
            return image, name, f"Registration Date: {created_date}", f"Approval Date: {approval_date}", f"Car Brand: {car_brand}", f"Car Model: {car_model}", f"Plate No.: {plate_number}", f"{ytd_count}",  f"{mtd_count}", f"{remaining_credit}"
